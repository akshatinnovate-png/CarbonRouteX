 ...
 * into a fully costed, constraint-checked fleet plan.
 *
...
export class PlanEngine {
  constructor(world, router, ctx) {
    this.world = world;
    this.router = router;
    this.ctx = ctx;               // { depots, ordersById, vehiclesById }
   ...
   * Returns a route object, always — an infeasible route is returned with
   * `feasible:false` and a populated `violations` array rather than thrown,
   * because the optimiser needs to be able to *score* infeasibility.
   */
  evaluateRoute(vehicle, orderIds, weights, startMinutes = SIM.dayStartMinutes) {
    const key = `${vehicle.id}|${orderIds.join(',')}|${JSON.stringify(weights)}|${this.router.traffic.revision}|${Math.round(startMinutes / 15)}`;
    const cached = this.routeCache.get(key);
    if (cached) return cached;
    const route = this._evaluateRouteUncached(vehicle, orderIds, weights, startMinutes);
  ...
  _evaluateRouteUncached(vehicle, orderIds, weights, startMinutes) {
    this.evaluations++;
    ...
    const route = {
      ...
      units: 0, co2: 0, cost: 0, toll: 0, risk: 0,
    ...
    if (!vehicle.available) {
      ...
      route.violations.push({ code: 'vehicle_unavailable', label: `${vehicle.callsign} is not available`, severity: 'hard' });
    }
    if (!orders.length) {
      route.empty = true;
      return route;
    ...
    // Departure is delayed by dock loading proportional to the consignment.
    let clock = startMinutes + OPTIMIZER.depotLoadMinutes + totalWeight * OPTIMIZER.serviceMinutesPerKg * 60 / 60;
    let remainingKg = totalWeight;
    let node = depot.nodeId;
    const poly = [];
    ...
    for (const step of sequence) {
      ...
      const profile = this.router.profile({
        vehicleType: vehicle.type, payload, weights, clockMinutes: clock,
      });
      const targetNode = step.kind === 'order' ? step.order.nodeId : depot.nodeId;
      const leg = this.router.path(node, targetNode, profile);
      if (!leg) {
        route.feasible = false;
        route.violations.push({
          code: 'unreachable', severity: 'hard',
          label: step.kind === 'order'
            ? `No open road to ${step.order.id} (${step.order.district})`
            : 'No open road back to depot',
        });
        break;
      }
      route.legs.push({ ...leg, toNode: targetNode, orderId: step.kind === 'order' ? step.order.id : null });
      for (let i = poly.length ? 1 : 0; i < leg.polyline.length; i++) poly.push(leg.polyline[i]);
      ...
      route.toll += leg.toll;
      route.risk += leg.risk;
      route.worstCongestion = Math.max(route.worstCongestion, leg.worstCongestion);
      clock += leg.minutes;
      node = targetNode;
      ...
      if (step.kind === 'order') {
        ...
        const waited = Math.max(0, o.windowOpen - arrival);
        const serviceStart = arrival + waited;
        const service = OPTIMIZER.serviceMinutesPerStop + o.serviceMinutes;
    ...
    // Energy feasibility against the vehicle's *current* state of charge.
    ...
    if (route.endMinutes > depot.closeMinutes) {
      route.violations.push({
        code: 'depot_close', severity: 'soft',
        label: 'Returns after depot closing time',
      });
    }
  ...
  buildPlan(assignment, weights, meta = {}) {
    ...
    for (const [vehicleId, orderIds] of assignment) {
      ...
      if (!vehicle) continue;
      const route = this.evaluateRoute(vehicle, orderIds, weights, meta.startMinutes ?? SIM.dayStartMinutes);
      routes.push(route);
      for (const id of orderIds) servedIds.add(id);
    }
    const unserved = [];
    for (const o of this.ctx.ordersById.values()) {
      if (o.status === 'delivered' || o.status === 'cancelled') continue;
      if (!servedIds.has(o.id)) unserved.push(o.id);
    }
...
export function finalisePlan(routes, unserved, weights, ctx, meta = {}) {
  ...
  const m = {
    km: 0, minutes: 0, drivingMinutes: 0, co2: 0, cost: 0, units: 0, toll: 0,
  ...
  for (const r of routes) {
    ...
    m.co2 += r.co2; m.cost += r.cost; m.units += r.units; m.toll += r.toll;
    m.stops += r.stops.length;
    m.lateMinutes += r.lateMinutes; m.lateOrders += r.lateOrders;
    m.payloadKg += r.capacityUsedKg;
    m.capacityKg += VEHICLE_TYPES[r.vehicleType].capacityKg;
...
/**
 * Reference scale used to normalise raw metrics into comparable 0..~2 numbers.
 * Derived from a reference plan (normally the unoptimised baseline) so that a
 * score of 1.0 means "as good as the baseline" and lower is better.
 */
 ...
 *
 *   score = w_time  * time/ref
 *         + w_cost  * cost/ref
 *         + w_co2   * co2/ref
 *         + w_dist  * km/ref
 *         + w_rel   * (1 - reliability * onTime)
 *         + lateness penalty
 *         + unserved penalty
 *         + hard-violation penalty
 *
 * Lower is better. Every term is reported back in `breakdown` so the
 * explanation layer can attribute a decision to a term rather than invent one.
 */
...
/** Quick feasibility test used inside the insertion heuristic's hot loop. */
